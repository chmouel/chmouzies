...
(defun ppr-make-pull-request (origin-remote user-remote &optional origin-remote-branch pushforce)
  (interactive
   (list (magit-read-remote "Base Remote")
         (magit-read-remote "User Remote")))
  (let* ((github-origin-remote (replace-regexp-in-string
    ...
    (if (or pushforce
            (yes-or-no-p
             (format
              "Push force current branch %s to user remote %s"
              current-branch
              user-remote)))
        (magit-run-git
         "push" "-f" "-v"
...
(ppr-make-pull-request "origin" "chmouel" "master" t)
